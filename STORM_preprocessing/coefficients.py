...
def track_coefficients():
    """
    Calculate the track coefficients, largely for the James & Mason functions. This function returns a npy-file with the coefficients
    per 5 deg latitude bin for every basin. 
    """
    step=5.
          
    data=np.load(os.path.join(dir_path,'STORM_variables','TC_TRACK_VARIABLES.npy'),allow_pickle=True).item()
    
    coefficients_list={i:[] for i in range(0,6)} #basins
    
    
    for idx in range(0,6):
           
        df=pd.DataFrame({'Latitude':data[4][idx],'Longitude':data[5][idx],'Dlat0':data[0][idx],'Dlat1':data[1][idx],'Dlon0':data[2][idx],'Dlon1':data[3][idx]}) #see preprocessing 
        lat0,lat1,lon0,lon1=preprocessing.BOUNDARIES_BASINS(idx) #get basin bounds for basin
        
        if idx==1:
            df=df[(df['Latitude']<=lat1) & (df['Latitude']>=lat0) & 
            ((df['Longitude']<360) & (df['Longitude']>=lon0) |(df['Longitude']<=lon1%360) & (df['Longitude']>=0)) ] #grab values within the basin
           
            df.loc[df['Dlon0']<-100,'Dlon0']=df.loc[df['Dlon0']<-100,'Dlon0']+360 #fix extremely negative values due to crossing dateline
            df.loc[df['Dlon1']<-100,'Dlon1']=df.loc[df['Dlon1']<-100,'Dlon1']+360
            
        else:    
            df=df[(df['Latitude']<=lat1) & (df['Latitude']>=lat0) & (df['Longitude']<=lon1) & (df['Longitude']>=lon0)] #grab values within the basin
    ...
    np.save(os.path.join(__location__,'../STORM_variables','TRACK_COEFFICIENTS.npy'),coefficients_list)                    
