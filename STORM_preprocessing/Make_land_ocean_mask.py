...
This part is needed for the creation of the land-ocean mask.
...
def run_landmask():
    ...
    land_mask_path = os.path.join(dir_path,'STORM_variables','LAND_MASK')
    if os.path.isdir(land_mask_path)==False:
        os.mkdir(land_mask_path) 
    
    for idx in range(0,6):
        print(idx)
        ocean_mask=Generate_landmask(idx)
        np.savetxt(os.path.join(land_mask_path,'Land_ocean_mask_'+str(idx)+'.txt'),ocean_mask)                