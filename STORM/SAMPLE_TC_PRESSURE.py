...
def find_index_pressure(idx,lat,lon,lat0,lon0,lon1):
    """
...
def haversine(lat1,lon1,lat2,lon2):
    ...
    dlon=abs(lon1-lon2)
    dlat=abs(lat2-lat1)
    A1=np.sin(dlat/2)**2.+np.cos(lat1)*np.cos(lat2)*np.sin(dlon/2)**2.
    C2=2.*np.arcsin(np.sqrt(A1))
    r=6371.
    km=C2*r
...
def distance_from_coast(lon,lat,fpath,degree_in_km=111.12):
    ...
    mindist=np.min(haversine(lat,lon,lats,lons))
    return mindist  

def add_parameters_to_TC_data(pressure_list,wind_list,latfull,lonfull,year,storm_number,monthlist,genesis_day,genesis_hour,landfallfull,lijst,TC_data,idx):
    """
    ...
    genesis_day : day of TC formation
    ...
    for l in range(0,x):
        if landfallfull[l]==1.:
            ...
            distance=distance_from_coast(lonfull[l],latfull[l],(os.path.join(dir_path,'STORM','coastal_basemap_data.npy')))
...
def TC_pressure(idx,latlist,lonlist,landfalllist,year,storms,monthlist,daylist,hourlist,TC_data):  
    ...
    monthlist : months of TC occurrence.
    daylist : days of TC formation.
    hourlist : hours of TC formation.
    TC_data : array of TC data.
    ...
    latidx_penv=np.linspace(90,-90,721) #latitude indices 
    lonidx_penv=np.linspace(0,359.75,1440)    #longitude indices
    
    JM_pressure=np.load(os.path.join(__location__,'../STORM_variables','COEFFICIENTS_JM_PRESSURE.npy'),allow_pickle=True).item()
            
    Genpres=np.load(os.path.join(__location__,'../STORM_variables','DP0_PRES_GENESIS.npy'),allow_pickle=True).item()
            
    WPR_coefficients=np.load(os.path.join(__location__,'../STORM_variables','COEFFICIENTS_WPR_PER_MONTH.npy'),allow_pickle=True).item()
    
    Genwind=np.load(os.path.join(__location__,'../STORM_variables','GENESIS_WIND.npy'),allow_pickle=True).item()
    ...
    wind_threshold=15.8 #if vmax<15.8, the storm is a tropical depression and we stop tracking it.
    
    
    for storm_number,genesis_day,genesis_hour,latfull,lonfull,landfallfull in zip(range(0,int(storms)),daylist,hourlist,latlist,lonlist,landfalllist):
        i=0
        vmax=0
        count=0
        p=np.nan
        
        month=monthlist[storm_number][0]
        ...
        while i<len(latfull): 
            ...
            if idx==1:
                cond_2=(lon0<=lon<=360) or (0 <=lon<=lon1)
            else:
                cond_2=lon0<=lon<=lon1
                
            if lat0<=lat<=lat1 and cond_2: #make sure we're inside the basin 
                
                if ((p<p_threshold) | math.isnan(p)): #something went wrong. start again
                    if i!=0:
                        print("fails",year,storm_number)
                    i=0
                    vmax=0
                ...
                if landfall==1: #landfall --> use Kaplan and DeMaria Formula for dissipation
                    if ((p<p_threshold) | math.isnan(p)):
                        ...
                        TC_data=add_parameters_to_TC_data(pressure_list,wind_list,latfull,lonfull,year,storm_number,monthlist[storm_number],genesis_day,genesis_hour,landfallfull,pressure_list,TC_data,idx)
                        i=100000000000000000000 
                        
                    else:
                       #calculate the landfall pressure
                        
                      ind=int(find_index_pressure(idx,lat,lon,lat0,lon0,lon1)) #find index for pressure
                    ...
                    elif i<len(latfull): #the storm does not move back over open ocean, so use the decay function until the storm has dissipated
                        #Extra condition added to ensure double calling does not occur
