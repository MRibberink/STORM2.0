...
def Check_NA_formation(lat,lon): 
    ...
    """
    if lat<=90. and lat>17.5 and lon>260.:
        l=1
    elif lat<=17.5 and lat>15. and lon>270.:
        l=1
    elif lat<=15. and lat>10. and lon>275.:
        l=1
    elif lat<=10. and lat>9. and lon>276.:
        l=1
    elif lat<=9. and lat>5. and lon>283.:
        l=1
    else:
        l=0
    return l

def Check_if_landfall(lat,lon,idx,land_mask):
    """
    Parameters
    ----------
    ...
    """
    lat0_WMO,lat1_WMO,lon0_WMO,lon1_WMO=Basins_WMO(idx)
...
def Startingpoint(no_storms,monthlist,daylist,hourlist,idx):
    """
    ...
    daylist : days on which TCs were formed
    hourlist : hour on which TCs were formed
    basin : basin.
    ...
    land_mask=np.loadtxt(os.path.join(dir_path,'STORM_variables','LAND_MASK','Land_ocean_mask_'+str(idx)+'.txt'))

    for month in monthlist:
        
        grid_copy=np.loadtxt(os.path.join(dir_path,'STORM_variables/GRID_GEN','GRID_GENESIS_MATRIX_'+str(idx)+'_'+str(month)+'.txt'))
        ...
        while var==0:
            ...
            if lon<lon1 and lat<lat1:
                check=Check_if_landfall(lat,lon,idx,land_mask) #make sure the coordinate isn't on land
